...
- **Status ID**: FR-204
- **체크리스트**
  - [ ] 층별장 검사 화면에서 `만료`, `검사 후 수정` 두 가지 토글 필터를 제공하고 `updatedAfterInspection` 플래그를 검사 흐름에서 일관되게 사용.
...
- **목표**: 검사 진행 중 필요한 상태(만료, 검사 후 수정)를 빠르게 추려 보고 벌점 안내 흐름과 연동한다.
...
- **체크리스트**
  ...
  - [ ] 활동 발생 시 `lockedUntil`이 30분씩 연장되고 추가 활동이 없으면 30분 후 자동 해제되는 잠금 정책을 검증하며, 관리자 강제 종료 플로우와 폴링 기반 재조회 전략(거주자/관리자 30~60초 간격)이 정상 동작하는지 확인한다.
