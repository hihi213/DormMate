...
## S0A 계정·승인 준비 (mvp-scenario.md §2)

### AU-101 가입 요청·승인 백엔드
- **Status ID**: AU-101
- **체크리스트**
  - [ ] `POST /auth/signup`에서 `signup_request`를 `PENDING`으로 저장하고 중복 요청 차단 및 유효성 검증을 실시.
  - [ ] 관리자 승인/반려 API를 구현해 승인 시 `dorm_user` 활성화·역할 매핑, 반려 시 사유 기록 및 재신청 제한 로직을 적용.
  - [ ] 비밀번호 변경 시 기존 세션 무효화, 승인 전 로그인 차단 등 인증 제약을 단위/통합 테스트로 확보.
  - [ ] *(Post-MVP 연계)* 비밀번호 변경 기능 도입 시 `user_session` 전량 `REVOKED` 처리 로직과 테스트를 연결한다.
- **Feature 링크**: [Feature Inventory §1 계정 및 권한](feature-inventory.md#1-계정-및-권한)
- **목표**: 거주자가 가입을 신청하고 관리자가 승인/반려를 처리할 수 있는 백엔드 흐름이 완성된다.
- **참고**: [feature-inventory.md §1](feature-inventory.md#1-계정-및-권한), [data-model.md §4](data-model.md#4-냉장고-도메인)

### AU-102 가입/승인 UI & 관리자 워크플로우
- **Status ID**: AU-102
- **체크리스트**
  - [ ] 다단계 가입 폼을 실제 API에 연결해 승인 대기·반려 결과 토스트/상태 화면을 구현하고, 중도 저장/재시도 UX를 보강.
  - [ ] 관리자 화면에서 가입 요청 목록·필터·승인/반려 액션과 결과 알림을 제공하며, 처리 로그를 [docs/ops/status-board.md](ops/status-board.md)에 기록.
  - [ ] 가입→승인→첫 로그인 흐름에 대한 Playwright/E2E 또는 수동 검증 리포트를 남긴다.
- **Feature 링크**: [Feature Inventory §1 계정 및 권한](feature-inventory.md#1-계정-및-권한)
- **목표**: 거주자 가입과 관리자 승인 플로우가 UI에서 완결되고 사용자에게 상태가 명확히 안내된다.
...
- **체크리스트**
  ...
  - [ ] 프런트 검사 관리 페이지에서 잠금 상태 표시와 거주자 화면 버튼 비활성화를 구현하고, 로컬 Draft 저장/정리 흐름이 정상 동작하는지 확인한다.
...
### IN-307 서버 Draft API 전환 *(Plan)*
- **Status ID**: IN-307
- **상태**: Post-MVP 전환 계획
- **체크리스트**
  - [ ] 검사 조치 초안을 서버 Draft API로 저장/복원하는 방식을 설계하고, 현재 로컬스토리지 기반 임시 저장 로직을 점진적으로 대체한다.
  - [ ] 다중 검사자 환경에서 실시간 Draft 동기화가 가능한지 검토하고, 충돌 해결 전략을 문서화한다.
  - [ ] Draft API 도입 시 로컬 초안 데이터 정리 및 마이그레이션 전략을 마련해 배포 전 사용자 잔여 데이터를 정리한다.
- **목표**: 임시 저장 로직을 서버 관리 방식으로 전환해 다중 검사자·브라우저간 협업을 지원할 준비를 한다.
- **참고**: [feature-inventory.md §4 – 실시간 협업 및 복구](feature-inventory.md#실시간-협업-및-복구)
...
- **Status ID**: SC-401
- **체크리스트**
  - [ ] 검사 일정/이력 저장을 위한 테이블과 CRUD API를 작성해 로컬 스토리지 의존성을 제거한다.
