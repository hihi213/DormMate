...
- 시나리오를 선택할 때는 `docs/mvp-scenario.md`에서 해당 단계(S0~S5, SX)를 확인하고 아래 Status ID를 사용자에게 재진술해 합의한다.
...
## 권장 진행 순서
1. EN-101 → EN-102 (S0 공통 준비)
2. FR-201 → FR-202 → FR-203 (S1 거주자 포장 관리)
3. IN-301 → IN-302 → IN-303 (S2 층별장 검사 세션)
4. NO-401 → NO-402 → NO-403 (S3 검사 결과 알림)
5. NO-501 → NO-502 (S4 임박·만료 자동 알림)
6. AD-601 → AD-602 (S5 관리자 운영 통제)
7. RG-701 → RG-702 (SX 회귀 & 산출물)
...
- **Status ID**: FR-201
- **체크리스트**
  - [ ] `GET /fridge/slots`가 배정 칸만 반환하는지 검증(`FridgeService#getSlots`).
  - [ ] 프런트 슬롯 선택 UX가 배정 칸만 표시하도록 API 파라미터/필터를 적용.
  - [ ] 데모 계정·칸 매핑과 라벨 시퀀스 초기화 상태 점검.
...
- **참고**: `docs/mvp-scenario.md` §3.1 1~4단계.
...
- **참고**: `docs/mvp-scenario.md` §3.1 5~6단계.
...
- **참고**: `docs/feature-inventory.md` §6, `docs/mvp-scenario.md` §3.2 1~2단계.
...
- **참고**: `docs/mvp-scenario.md` §3.2 3~4단계, `docs/data-model.md` §4.3.
...
- **참고**: `docs/mvp-scenario.md` §3.2 5~6단계.
...
- **참고**: `docs/mvp-scenario.md` §3.3 1~3단계.
...
- **참고**: `docs/mvp-scenario.md` §3.3 4단계.
