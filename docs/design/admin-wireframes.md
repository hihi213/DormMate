...
## 개정 이력
- 2025-11-08: 실제 릴리스 스코프 요약 추가 및 미구현 섹션 표기
...
- 2025-11 배포 기준 라이브 관리 기능은 `냉장고 슬롯·포장`, `검사 이력 및 정정`, `알림 확인`, `데모 데이터 초기화` 네 축으로 구성되어 있다. 관련 UI/동작은 섹션 3~7과 17에서 상세히 다룬다.
- 섹션 8~14는 세탁실·도서관·다목적실·벌점·운영 지원 등 향후 확장 모듈을 위한 설계 초안으로, 아직 FE/BE가 구현되지 않았다. 실무 참고 시 “향후 모듈”임을 명시하고 사용한다.
- 모든 상세 스냅샷과 컴포넌트 구조는 Next.js + shadcn UI 기반 실제 관리자 화면과 동기화되어 있으며, 디자인 변경 시 섹션 17을 우선 업데이트한다.
...
- **냉장고 모듈**: 모듈 헤더(층/칸 필터) + 상단 탭 `층별장 관리`, `냉장고 관리`로 섹션 3~4 화면을 전환하고, 심화 조사는 감사 로그 & 리포트 프리셋으로 연결한다. 기본 레이아웃은 좌측 리스트/카드 + 우측 세부 패널.
  ...
  - **시스템 설정**: 탭 `냉장고 관제`, `벌점 정책`, `모니터링`, `환경 변수`로 구성. 각 탭은 카드 그리드 + 상세 패널(섹션 7).
...
- 필터: 층 선택(전체 포함), 계정 상태 토글(활성/비활성), 검색(이름·호실·개인번호·사용자 ID)
- 요약 레이아웃: 상단 배너에서 총 인원수를 배지로 표시하고, 승격/비활성화 선택 모드를 버튼으로 제공한다.
- 목록 상단 툴바: 검색 폼과 상태 토글, `층별장 임명`·`비활성화` 선택 모드를 지원한다. 선택 모드가 켜진 동안에만 각 행의 체크박스가 활성화된다.
- 요약 목록(테이블)
  - 열 구성: 호실 코드, 개인번호, 사용자 정보(이름+호실/개인번호 텍스트), 추가 권한 배지(층별장), 벌점, 최근 로그인, 상태 배지, 상세 보기 버튼
  - 페이지당 6명, 기본 정렬은 최근 로그인 기준. 페이징 컨트롤은 카드 하단에 배치한다.
  - 행 클릭 시 우측 드로어가 열리고 `기본 역할`, `호실·개인번호`, `추가 권한`, `누적 벌점` 요약과 층별장 임명/해제, 비활성화 버튼을 노출한다.
- **핵심 기능**
  - 호실·권한 변경은 다른 모듈에서 수행하지만, DTO(`roles`, `status`, `penalties`)를 이용해 이 화면에서도 최신 상태를 한눈에 파악할 수 있어야 한다.
  - 승격/비활성화는 현재 선택 모드→토스트 알림 수준으로 안내하고, 후속 워크플로우(권한 관리/계정 잠금)는 추후 API 연동으로 확장한다.
  - 벌점 조정·알림 재전송 등 추가 액션은 드로어 섹션을 확장하는 방향으로 후속 설계를 진행한다.
...
## 17. 현재 구현된 관리자 UI 스냅샷 (2025-11 기준)

아래 내용은 실제 관리자 앱(Next.js + Shadcn UI)에서 확인할 수 있는 상태를 요약한 것으로, 와이어프레임과 구현 간 차이를 빠르게 파악하기 위한 참고용이다.

1. **헤더·필터**
   - 상단 헤더에는 좌측 로고, 중앙 전역 검색, 우측 알림 벨·프로필 드롭다운이 실제로 구현되어 있다.
   - 냉장고 모듈 상단에는 `층 선택(2F~5F)` 드롭다운과 `슬롯 검색` 인풋, `재배분`, `검사 일정`, `데모 초기화` 버튼이 한 줄에 배치되어 즉시 조치가 가능하다.

2. **칸/포장 관리 카드**
   - `/admin/fridge` 메인 영역은 층별 칸 카드 그리드로 구성돼 있으며, 각 카드에 상태 배지(운영/잠금/보고)와 점유 수(`occupiedCount`)가 실시간 표시된다.
   - 카드를 클릭하면 우측 패널에서 칸 상세(할당 호실, 포장 목록, 경고 상태)를 확인할 수 있고, `칸 설정`, `검사 보기`, `포장 열기` CTA가 패널 상단에 고정된다.

3. **포장/물품 패널**
   - 우측 패널은 `포장 목록`, `삭제 이력`, `알림` 탭으로 구성되어 실제 CRUD 버튼(메모 수정, 강제 삭제, 라벨 재발급)이 그대로 노출된다.
   - 삭제 이력 탭에선 `라벨 재사용` 배지가 함께 표시되고, 검색 인풋은 스티커 번호/주민 이름/칸 코드 등 복합 키워드를 지원한다. 삭제 이력 전용 API(`GET /admin/fridge/bundles/deleted`)가 연결돼 있어 페이지/기간 필터가 바로 반영된다.

4. **검사 이력 & 정정 UI**
   - 하단 `최근 검사` 섹션은 표 형태의 검사 목록과 카드형 요약을 지원하며, 행을 클릭하면 다이얼로그가 열려 조치 타임라인·벌점 상태·알림 전송 내역을 보여 준다.
   - 다이얼로그에는 `정정` 버튼과 `알림 재발송`(현재는 안내 토스트만 노출)만 남아 있으며, 재검 요청 버튼은 미구현 상태라 UI에서 제거됐다. 정정은 `/fridge/inspections/{id}` PATCH와 직접 연동되어 메모/조치 수정·삭제를 수행하고, 성공 시 “감사 로그에 기록되었습니다” 토스트가 뜬다.
   - 정정 모달은 조치 리스트 + 노트 에디터로 구성되어 있고, 조치 삭제 시 시각적 강조(빨간 박스)와 복구 토글을 지원한다.

5. **검사 일정·재배분 모달**
   - `검사 일정 관리` 버튼은 팝업에서 일정 리스트와 생성/수정 폼을 제공하며, 모든 필드는 서버 유효성(충돌 감지) 결과를 토스트로 안내한다.
   - `호실 재배분` 다이얼로그는 층별 칸 카드와 호실 체크박스가 포함된 2단 레이아웃으로 구현되어, 권장 배정/현재 배정을 비교한 뒤 적용할 수 있다. 경고(warnings) 뱃지가 잠금/검사 중 칸을 즉시 표시한다.

6. **데모 초기화**
   - `데모 데이터 초기화` 버튼이 관리자 도구 섹션에 고정되어 있으며, 실행 시 `/admin/seed/fridge-demo` API 호출 + 로컬 스토리지 캐시 초기화를 수행한다. 실행 결과는 “데모 데이터가 갱신되었습니다” 토스트로 안내되고, UI에는 `전시 데모:` 포장이 즉시 나타난다.

7. **반응형 구현**
   - 1280px 이상에서는 카드+패널 2열 구조, 960~1279px에서는 패널이 슬라이드 형태로 전환되고, 모바일에서는 카드 내부 아코디언이 순차적으로 펼쳐지도록 구현됐다.

### 17.1 새 기능 요약

| 기능 | UI 위치 | 구현 메모 |
| --- | --- | --- |
| 검사 정정(PATCH) | 검사 상세 다이얼로그 → `정정` | 메모/조치 추가·수정·삭제 지원, 저장 시 `INSPECTION_ADJUST` 토스트 표시 |
| 알림 재발송 | 검사 상세 다이얼로그 → `알림 재발송` 버튼 | 후속 API 연동 예정, 현재는 Placeholder 토스트 |
| 데모 데이터 초기화 | 관리자 헤더 우측 도구 / `Danger Zone` 카드 | `/admin/seed/fridge-demo` 호출, 실행 후 Slot/검사 카드 즉시 갱신 |
| 재배분 경고 | `호실 재배분` 다이얼로그 카드 | warnings 배열을 그대로 뱃지로 표시, 잠금/검사 중 칸을 강조 |

위 구성이 현재 FE/BE 코드와 일치하므로, 향후 디자인 변경 시 본 섹션을 갱신해 실서비스와 문서 간 싱크를 유지한다.
