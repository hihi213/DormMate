...
4. **검사 이력 & 정정 UI**
   ...
   - 다이얼로그에는 `정정`, `알림 재발송`, `재검 요청` 버튼이 존재한다. 특히 정정 버튼은 `/fridge/inspections/{id}` PATCH와 직접 연동되어 메모/조치 수정·삭제를 수행하고, 성공 시 “감사 로그에 기록되었습니다” 토스트가 뜬다.
...
| 재검 요청 | 검사 상세 다이얼로그 → `재검 요청` | 일정 생성 팝업으로 이동, 일정 저장 시 감사 로그 연동 |
