...
- 요약 목록(테이블)
  ...
  - 행 클릭 시 우측 드로어가 열리고 `기본 역할`, `호실·개인번호`, `추가 권한`, `진행 중 검사`, `누적 벌점` 요약과 층별장 임명/해제, 비활성화 버튼을 노출한다.
- **핵심 기능**
  - 호실·권한 변경은 다른 모듈에서 수행하지만, DTO(`roles`, `status`, `inspectionsInProgress`, `penalties`)를 이용해 이 화면에서도 최신 상태를 한눈에 파악할 수 있어야 한다.
