...
*용도: [2.Demo_Scenario.md](2.Demo_Scenario.md) 단계(S0~S5)를 실제 시연하기 위한 할 일·테스트 체크리스트다. 각 블록은 [docs/2.2.Status_Board.md](2.2.Status_Board.md)에서 WIP/DONE으로 추적되는 Status ID와 연결된다. 상세 흐름/멘트는 시나리오 문서에 위임한다.*

> 목적: 시연 플로우를 안정적으로 구현하면서도 확장 대비 코드를 보존하기 위해, 백엔드·프런트·테스트 관점의 핵심 작업을 시나리오 흐름 순서대로 정의한다.

## 사용 가이드
- 진행·기록 상세 절차는 [docs/2.2.Status_Board.md](2.2.Status_Board.md)를 따른다.
- 참고의 [2.Demo_Scenario.md](2.Demo_Scenario.md)에서 실제 시연 단계(S0~S5, SX)를 확인하고 아래 Status ID를 사용자에게 재진술해 합의한다.
- 각 Status 착수 전 [2.Demo_Scenario.md](2.Demo_Scenario.md) 해당 절과 [1.Feature_Inventory.md](1.Feature_Inventory.md) 연결 섹션을 읽고, 이해한 근거를 [docs/2.2.Status_Board.md](2.2.Status_Board.md)에 명시한다.
- **착수 전 공통 스캔 루틴** – 어떤 Status든 시작하기 전에 아래 자료를 순서대로 훑어보고 핵심 포인트를 메모한다.
  1. [2.Demo_Scenario.md](2.Demo_Scenario.md)에서 해당 시나리오 구간을 확인해 전체 흐름과 데모 요구사항을 정리한다.
  2. [1.Feature_Inventory.md](1.Feature_Inventory.md)에서 모듈별 정책·예외·상태 정의를 찾아 코드/테스트에 반영할 체크리스트를 만든다.
  3. [data-model.md](data-model.md)에서 연관 테이블·필드·상태값을 리뷰하고, 관련 마이그레이션/도메인 엔티티 파일 위치를 메모한다.
  4. 필요 시 [docs/ai-impl/backend.md](ai-impl/backend.md), [docs/ai-impl/frontend.md](ai-impl/frontend.md)의 구현 지침과 기존 [docs/2.2.Status_Board.md](2.2.Status_Board.md) 로그를 참고해 이전 결정 사항을 상기한다.
  5. 상기 내용을 [docs/2.2.Status_Board.md](2.2.Status_Board.md) WIP 섹션에 근거로 기록한 뒤 구현에 착수한다.
...
- 정책·데이터 모델이 불명확할 때는 [1.Feature_Inventory.md](1.Feature_Inventory.md), [data-model.md](data-model.md)를 교차 참고해 문서를 업데이트한다.
- API·스키마를 수정할 때는 코드뿐 아니라 `api/openapi.yml`과 관련 문서(플랜/시나리오/인벤토리)를 함께 갱신한다.
- 이 문서의 체크리스트는 기능 구현을 위한 **최소 요구사항**만 기술한다. 실제 개발 시에는 각 Status의 *Feature 링크*를 따라가 [1.Feature_Inventory.md](1.Feature_Inventory.md) 원문과 연계 문서를 충분히 이해한 뒤, 필요한 세부 기능/예외/테스트 항목을 이 플랜에 확장 기록하면서 구현한다.
- “Stretch” 표시(Status 예: IN-306)는 MVP 완료 후 시간 여유가 있을 때 진행하며, 상태 보드에는 Backlog/차기 일정으로 구분한다.
- **예외 코드 기준**: 용량/잠금/검사 충돌 등 핵심 흐름에서 반환되는 HTTP 상태/에러 코드(422 `CAPACITY_EXCEEDED`/`CAPACITY_BELOW_ACTIVE`, 423 `COMPARTMENT_LOCKED`/`COMPARTMENT_UNDER_INSPECTION`, 409 `COMPARTMENT_HAS_ACTIVE_SESSION`, 400 `INVALID_*`)를 체크리스트에 명시해 테스트·데모 근거를 남긴다.

## S0 공통 준비 (2.Demo_Scenario.md §2)
...
- **체크리스트**
  ...
  - [x] `deploy/.env.prod` 최신화: `JWT_SECRET` 필수, `CORS_ALLOWED_ORIGINS` 지정, 포트(로컬 8080/8443, 운영 80/443) 확인.
  - [x] Docker Compose 기동 경로 정리: `docker compose --env-file deploy/.env.prod -f docker-compose.yml -f docker-compose.prod.yml up -d proxy` (proxy up 시 app/frontend 동시 기동). TLS 필요 시 `./auto deploy tls issue|renew --env prod`.
  - [x] 데모 seed(`/admin/seed/fridge-demo`)는 스테이징/데모 전용, 운영 금지 경고 반복 표기.
- **목표**: 데모/로컬 환경에서 동일한 데이터로 기동·롤백을 수행할 수 있다.
- **참고**: [2.Demo_Scenario.md §2](2.Demo_Scenario.md#2-사전-준비), [1.Feature_Inventory.md §2](1.Feature_Inventory.md#2-냉장고--일반-기숙사생)
...
- **참고**: [docs/2.2.Status_Board.md](2.2.Status_Board.md#auth-201-로그인세션-안정화)
...
- **체크리스트**
  ...
  - [ ] 관련 테스트/문서 TODO를 `docs/2.2.Status_Board.md` Backlog에 남겨 추후 착수 시 참조한다.
- **Feature 링크**: [Feature Inventory §1 계정 및 권한](1.Feature_Inventory.md#1-계정-및-권한)
- **목표**: 실무 운영에 필요한 계정 복구·감사 추적 기능을 제공한다.
- **참고**: [1.Feature_Inventory.md §1](1.Feature_Inventory.md#1-계정-및-권한), [1.Feature_Inventory.md §6](1.Feature_Inventory.md#6-phase-2-모듈-개요-참고)

---

## S1 거주자 포장 관리 (2.Demo_Scenario.md §3.1)
...
- **Feature 링크**: [Feature Inventory §2 냉장고 – 일반 기숙사생](1.Feature_Inventory.md#2-냉장고--일반-기숙사생)
- **목표**: 거주자가 자신의 칸만 선택해 등록을 시작할 수 있다.
- **참고**: [data-model.md §4](data-model.md#4-냉장고-도메인), [2.Demo_Scenario.md §3.1](2.Demo_Scenario.md#31-거주자-포장-등록-및-관리)
...
- **참고**: [2.Demo_Scenario.md §3.1](2.Demo_Scenario.md#31-거주자-포장-등록-및-관리) 1~4단계 시나리오 최종 구현이 목표

### FR-203 임박/만료 · 라벨 재사용
- **Status ID**: FR-203
- **체크리스트**
  - [ ] `PATCH/DELETE /fridge/items/{id}` 소프트 삭제 후 라벨 재사용(`bundle_label_sequence`)을 검증하고, 동일 라벨 재발급이 API 응답에 반영되는지 확인한다.
  - [ ] 임박/만료 배지를 `freshness`(`ok/expiring/expired`) 기반으로 통합 표시하고 UI 문구·색상을 정책에 맞춰 조정한다.
  - [ ] `FridgeIntegrationTest`로 임박/만료·라벨 플로우 회귀 테스트.
  - [ ] 라벨 시퀀스 기본 범위(001~999) 및 `max_bundle_count` 관리자 조정 정책을 서버 단에서 검증하고 통합 테스트로 보호한다(422 `CAPACITY_EXCEEDED`, 422 `CAPACITY_BELOW_ACTIVE`).
...
- **참고**: [2.Demo_Scenario.md §3.1](2.Demo_Scenario.md#31-거주자-포장-등록-및-관리) 5~6단계 시나리오 최종 구현이 목표
...
- **Feature 링크**: [Feature Inventory §2 냉장고 – 일반 기숙사생](1.Feature_Inventory.md#2-냉장고--일반-기숙사생), [Feature Inventory §3 냉장고 – 알림 & 일정](1.Feature_Inventory.md#3-냉장고--알림--일정)
- **목표**: 검사 진행 중 필요한 상태(만료, 검사 후 수정)를 빠르게 추려 보고 벌점 안내 흐름과 연동한다.
- **참고**: [2.Demo_Scenario.md §3.3](2.Demo_Scenario.md#33-검사-후-사용자-알림)

### FR-205 데이터 시드
- **Status ID**: FR-205
- **체크리스트**
  - [x] `/admin/seed/fridge-demo` 관리자 API가 전시용 물품 시드를 교체하는 방식으로 동작하도록 정비하고, Flyway 정적 시드 의존을 제거한다.
  - [x] 전시·데모 데이터 초기화 절차를 `docs/2.Demo_Scenario.md`와 [docs/ops/README.md](ops/README.md)에 문서화하고 경고 문구를 반복 노출한다.
  - [ ] 운영/테스트 환경별로 필요한 추가 초기 데이터가 있다면 관리자 도구 또는 별도 API로 확장할 계획을 정리하고, 상태 보드에 TODO로 남긴다.
- **Feature 링크**: [Feature Inventory §2 냉장고 – 일반 기숙사생](1.Feature_Inventory.md#2-냉장고--일반-기숙사생), [Feature Inventory §4 냉장고 – 층별장(검사자)](1.Feature_Inventory.md#4-냉장고--층별장검사자)
- **목표**: 운영 환경에서 직접 DB를 수정하지 않고 초기 데이터를 준비/갱신할 수 있다.
- **참고**: [2.Demo_Scenario.md §2](2.Demo_Scenario.md#2-사전-준비)

---

## S2 층별장 검사 세션 (2.Demo_Scenario.md §3.2)

### IN-301 검사 시작·잠금
- **Status ID**: IN-301
- **체크리스트**
  - [ ] `POST /fridge/inspections` 접근 권한(층별장/관리자) 검증 및 세션 생성(201).
  - [ ] 세션 시작 시 칸 잠금(`FridgeCompartment.locked`, `lockedUntil`) 적용을 검증하고, 기존 세션이 종료된 뒤 신규 세션을 열 때 409/403이 아닌 정상 동작하도록 예외를 확인한다.
  - [ ] 프런트 검사 관리 페이지에서 잠금 상태 표시와 거주자 화면 버튼 비활성화를 구현하고, 자동/수동 로컬 Draft 저장·정리 흐름이 정상 동작하는지 확인한다.
  - [ ] 활동 발생 시 `lockedUntil`이 30분씩 연장되고 추가 활동이 없으면 30분 후 자동 해제되는 잠금 정책을 검증하며, 관리자 강제 종료 플로우와 폴링 기반 재조회 전략(거주자/관리자 30~60초 간격)이 정상 동작하는지 확인한다. 423 `COMPARTMENT_LOCKED`/`COMPARTMENT_UNDER_INSPECTION` 재현 포함.
- **Feature 링크**: [Feature Inventory §4 냉장고 – 층별장(검사자)](1.Feature_Inventory.md#4-냉장고--층별장검사자)
- **목표**: 검사 시작 시 칸이 잠기고 다른 사용자에게도 잠금 상태가 반영된다.
- **참고**: [2.Demo_Scenario.md §3.2](2.Demo_Scenario.md#32-층별장-검사-세션-단독-흐름) 1~2단계 시나리오 최종 구현이 목표

### IN-302 조치 선택·벌점 연동
- **Status ID**: IN-302
- **체크리스트**
  - [ ] `POST /fridge/inspections/{id}/actions`로 PASS/WARN/DISPOSE/UNREGISTERED_DISPOSE 조치를 저장하고, 실패 시 예외 코드(`INVALID_ACTION`, 403 `FORBIDDEN_SLOT`)를 확인한다.
  - [ ] DISPOSE/UNREGISTERED_DISPOSE 조치 시 벌점 1점이 `penalty_history`에 누적되고, 경고/폐기 통계 및 검사 결과 알림과 연동되는지 단위/통합 테스트를 작성한다.
  - [ ] 프런트 조치 UI에서 액션 코드 매핑과 미등록 물품 입력 플로우를 구현한다. 실시간 동기화(SSE)는 Post-MVP 확장(IN-306)으로 이월하고, 현재는 단독 검사 흐름 안정화에 집중한다.
- **Feature 링크**: [Feature Inventory §4 냉장고 – 층별장(검사자)](1.Feature_Inventory.md#4-냉장고--층별장검사자), [Feature Inventory §3 냉장고 – 알림 & 일정](1.Feature_Inventory.md#3-냉장고--알림--일정)
- **목표**: 층별장이 조치별로 기록을 남기고 폐기 시 벌점이 즉시 반영된다.
- **참고**: [2.Demo_Scenario.md §3.2](2.Demo_Scenario.md#32-층별장-검사-세션-단독-흐름) 3~4단계 시나리오 최종 구현이 목표

### IN-303 제출 요약·잠금 해제
- **Status ID**: IN-303
- **체크리스트**
  - [ ] `POST /fridge/inspections/{id}/submit`에서 통계 집계와 잠금 해제를 검증하고, 중복 제출/없는 세션에 대한 예외(404/409)를 확인한다.
  - [ ] 제출 시 경고/폐기/총 포장 수 요약을 모달 또는 토스트로 표시하고, 제출 직후 칸 잠금이 해제되는지 UI에서 재확인한다.
  - [ ] Playwright/수동 테스트로 제출 후 잠금 해제와 거주자 UI 복구(등록 버튼 활성화, 423 미발생)를 확인한다.
- **Feature 링크**: [Feature Inventory §4 냉장고 – 층별장(검사자)](1.Feature_Inventory.md#4-냉장고--층별장검사자)
- **목표**: 검사 완료 시 결과 요약을 보여주고 칸이 다시 편집 가능해진다.
- **참고**: [2.Demo_Scenario.md §3.2](2.Demo_Scenario.md#32-층별장-검사-세션-단독-흐름) 5~6단계 시나리오 최종 구현이 목표
...
- **체크리스트**
  ...
  - [ ] 거주자 열람 흐름에 대한 수동 또는 Playwright 검증 로그를 [docs/2.2.Status_Board.md](2.2.Status_Board.md)에 기록한다.
- **Feature 링크**: [Feature Inventory §2 냉장고 – 일반 기숙사생](1.Feature_Inventory.md#2-냉장고--일반-기숙사생), [Feature Inventory §4 냉장고 – 층별장(검사자)](1.Feature_Inventory.md#4-냉장고--층별장검사자)
- **목표**: 거주자가 자신의 칸 검사 일정과 결과를 확인하되 검사 실행 권한은 유지되지 않는다.
- **참고**: [2.Demo_Scenario.md §3.2](2.Demo_Scenario.md#32-층별장-검사-세션-단독-흐름)
...
- **Feature 링크**: [Feature Inventory §3 냉장고 – 알림 & 일정](1.Feature_Inventory.md#3-냉장고--알림--일정), [Feature Inventory §4 냉장고 – 층별장(검사자)](1.Feature_Inventory.md#4-냉장고--층별장검사자)
...
## S3 검사 결과 알림 (2.Demo_Scenario.md §3.3)

### NO-401 검사 결과 알림 발행
- **Status ID**: NO-401
- **체크리스트**
  - [ ] `NotificationService#sendInspectionResultNotifications` 단위/통합 테스트 확보.
  - [ ] `FRIDGE_RESULT:{sessionUuid}:{userId}` dedupe 키와 TTL(`admin_policy.notification_ttl_hours`)이 정책과 일치하는지 검증.
...
- **Feature 링크**: [Feature Inventory §3 냉장고 – 알림 & 일정](1.Feature_Inventory.md#3-냉장고--알림--일정)
...
- **Status ID**: NO-402
- **체크리스트**
  - [ ] 거주자 알림 목록(`GET /notifications?state=unread|all`)에서 `FRIDGE_RESULT`가 최상단에 노출되고, 읽음 처리(`/notifications/{id}/read`, `/notifications/read-all`) 후 배지가 제거되는지 확인한다.
  - [ ] 알림 상세 클릭 시 `correlationId`·`actionIds`를 이용해 검사 결과 상세/포장 화면으로 이동하고 권한 검증(403 시 메시지 안내)이 정상 동작하는지 확인한다.
  - [ ] 폐기 알림 상세에서 벌점 1점 누적 정보를 노출한다.
  - [ ] 알림 TTL 만료(`state=EXPIRED`) 시 목록에서 제거되는지, 만료 직전 읽음 처리 UX를 점검한다.
...
- **참고**: [2.Demo_Scenario.md §3.3](2.Demo_Scenario.md#33-검사-후-사용자-알림) 1~3단계 시나리오 최종 구현이 목표
...
- **체크리스트**
  ...
  - [ ] 알림 설정 저장용 백엔드 API(`PATCH /notifications/preferences/{kind}`)를 호출해 종류별 ON/OFF, 백그라운드 허용 여부를 업데이트하고 단위·통합 테스트로 검증한다.
  - [ ] 설정 화면에서 최근 발송 알림의 `correlationId`·상세 링크가 올바르게 작동하는지 수동/E2E 테스트 절차를 마련한다.
- **Feature 링크**: [Feature Inventory §3 냉장고 – 알림 & 일정](1.Feature_Inventory.md#3-냉장고--알림--일정)
- **목표**: 사용자가 알림 수신 여부를 직접 제어할 수 있다.
- **참고**: [2.Demo_Scenario.md §3.3](2.Demo_Scenario.md#33-검사-후-사용자-알림) 4단계 시나리오 최종 구현이 목표
...
- **참고**: [2.Demo_Scenario.md §3.3](2.Demo_Scenario.md#33-검사-후-사용자-알림), [2.Demo_Scenario.md §3.4](2.Demo_Scenario.md#34-임박만료-자동-알림)

### NO-405 알림 실패 로그·정책 관리
- **Status ID**: NO-405
- **체크리스트**
  - [ ] 알림 발송 실패 시 `notification_dispatch_log`에 원인과 상태가 기록되도록 서비스/리포지토리를 검증하고, 재시도 정책을 명시한다.
  - [ ] 알림 재시도/무시 정책을 운영 문서([docs/2.2.Status_Board.md](2.2.Status_Board.md))와 코드로 정리하고, 관리자 UI에서 실패 로그를 조회 가능하게 한다.
  - [ ] 하루 발송 상한·TTL 등 정책은 현재 `admin_policy`를 사용함을 명시하고, 추가 `notification_policy` 테이블 도입은 Post-MVP Backlog로 남긴다.
  - [ ] 실패 로그 화면에서 `slotId`·`correlationId`·시간 범위 필터로 특정 검사 알림을 추적할 수 있도록 하고, 동작 여부를 통합 테스트/QA 체크리스트에 포함한다.
- **Feature 링크**: [Feature Inventory §3 냉장고 – 알림 & 일정](1.Feature_Inventory.md#3-냉장고--알림--일정), [Feature Inventory §5 냉장고 – 관리자](1.Feature_Inventory.md#5-냉장고--관리자)
- **목표**: 알림 실패가 추적되고 정책 값이 운영 UI에서 관리 가능하다.
- **참고**: [2.Demo_Scenario.md §3.3](2.Demo_Scenario.md#33-검사-후-사용자-알림)

---

## S4 임박·만료 자동 알림 (2.Demo_Scenario.md §3.4)
...
- **참고**: [2.Demo_Scenario.md §3.4](2.Demo_Scenario.md#34-임박만료-자동-알림)
...
- **참고**: [2.Demo_Scenario.md §3.4](2.Demo_Scenario.md#34-임박만료-자동-알림) 1~3단계
...
- **Feature 링크**: [Feature Inventory §5 냉장고 – 관리자](1.Feature_Inventory.md#5-냉장고--관리자), [Feature Inventory §2 냉장고 – 일반 기숙사생 · 증설 정책](1.Feature_Inventory.md#냉장고-증설재배분)
- **목표**: 관리자가 칸 용량·상태를 조정하고 통계를 확인할 수 있다.
- **참고**: [2.Demo_Scenario.md §3.5](2.Demo_Scenario.md#35-관리자-운영-통제)
...
- **Feature 링크**: [Feature Inventory §5 냉장고 – 관리자](1.Feature_Inventory.md#5-냉장고--관리자)
- **목표**: 관리자/층별장 권한 변경을 UI에서 수행하고 검증할 수 있다.
- **참고**: [2.Demo_Scenario.md §3.5](2.Demo_Scenario.md#35-관리자-운영-통제) 2~3단계
...
## SX 회귀 & 산출물 (2.Demo_Scenario.md §4)

### RG-701 회귀 테스트
- **Status ID**: RG-701
- **체크리스트**
  - [ ] `./gradlew test`, `/actuator/health`, `/fridge/slots`, `/fridge/inspections/active` 스모크 호출.
  - [ ] 거주자→층별장→관리자 e2e 또는 수동 시나리오 실행 후 결과를 [docs/2.2.Status_Board.md](2.2.Status_Board.md)에 기록.
...
- **참고**: [2.Demo_Scenario.md §4](2.Demo_Scenario.md#4-마무리-및-배포-안정성-강조)
...
- **참고**: [2.Demo_Scenario.md §3.2](2.Demo_Scenario.md#32-층별장-검사-세션-단독-흐름), [2.Demo_Scenario.md §3.3](2.Demo_Scenario.md#33-검사-후-사용자-알림)

### RG-704 프런트 E2E/Escape 테스트
- **Status ID**: RG-704
- **체크리스트**
  - [ ] 현행 Playwright는 fixture 모드(smocked API) 기반 smoke/권한/취소 배지 3종만 존재함을 명시하고, 실행 명령(`npm run playwright:test`)과 실 서버 대상 실행 방법을 문서화한다.
  - [ ] 실 서버 기반 관리자/거주자 E2E는 미작성임을 표시하고, 필요 시 테스트 DB/모킹 전략을 정리한다.
  - [ ] 테스트 결과를 status-board에 기록하고 회귀 실패 시 대응 전략을 문서화한다.
- **목표**: 프런트 핵심 시나리오가 자동화 테스트로 검증되어 회귀 위험을 줄인다(실서버 E2E는 차기 과제).
- **참고**: [docs/tests/admin-playwright-plan.md](admin-playwright-plan.md), [docs/2.2.Status_Board.md](2.2.Status_Board.md).

---

## 시나리오 검증 가이드 (2.Demo_Scenario.md 요약)
...
> 위 시나리오는 각 기능(Status ID)이 구현된 후 데모/QA에서 어떻게 검증되는지를 요약한 것으로, 세부 단계는 `docs/2.Demo_Scenario.md`를 참고한다.
...
- **체크리스트**
  ...
  - [ ] EventSource 재연결/타임아웃 처리와 단독 모드 전환 시나리오를 통합 테스트 또는 Playwright 시나리오로 검증하고, 장애 발생 시 복구 지침을 [docs/2.2.Status_Board.md](2.2.Status_Board.md)에 기록한다.
- **Feature 링크**: [Feature Inventory §4 냉장고 – 층별장(검사자) · 실시간 협업](1.Feature_Inventory.md#실시간-협업-및-복구)
