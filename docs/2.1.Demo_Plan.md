...
- **체크리스트**
  ...
  - [x] Docker Compose 기동 경로 정리: `docker compose --env-file deploy/.env.prod -f docker-compose.yml -f docker-compose.prod.yml up -d proxy` (proxy up 시 app/frontend 동시 기동). TLS 필요 시 `./auto deploy tls issue|renew`.
