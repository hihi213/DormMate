...
- **데모 데이터**: 
  ...
  - 시연 직전 관리자는 `/admin/seed/fridge-demo` POST API를 호출해 표준 데모 데이터를 재시딩한다. 이 작업은 냉장고/검사 관련 테이블을 모두 초기화하므로, 운영 데이터가 있을 때는 **절대 실행하지 말 것**을 명시하고 진행 전 백업 여부를 확인한다. (근거: `backend/src/main/resources/db/demo/fridge_demo_refresh.sql`, [운영 매뉴얼 “데모 데이터 초기화”](ops/README.md#데모-데이터-초기화-관리자-전용))
...
1. 거주자 계정 로그인 → `내 냉장고` 진입. (보안을 보여 주기 위해 다른 브라우저에서 동일 계정으로 로그인 시도 후 `deviceId` 불일치로 세션이 폐기되는지 간단히 확인)
...
3. 검사 화면에서 문제 없는 포장은 `통과`, 임박 포장은 `경고`, 만료 포장은 `폐기`로 조치 선택한다. 조치 시 즉시 로컬 임시 저장이 이루어지고, 재접속해도 동일 상태가 복원됨을 간단히 보여 준다.
