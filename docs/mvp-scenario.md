...
2-1. 일반 거주자 계정으로 검사 화면에 접근했을 때 `일정 추가/검사 시작` 버튼이 숨겨지고 “층별장 전용 기능입니다.” 안내만 노출되는지 확인한다.
...
5. 검사 중에는 활동이 있을 때마다 잠금이 30분씩 연장되고, 추가 활동이 없으면 30분 후 자동 해제된다는 정책을 안내한다. 관리자만 `강제 종료`로 즉시 해제할 수 있으며, 백엔드가 `lockedUntil`을 폴링 기반으로 정리(만료 시 자동 해제)하도록 설계됐음을 설명한다.
6. `검사 제출` 시 통계 요약(경고/폐기/총 포장 수) 확인, 칸 잠금 해제. 제출과 동시에 `localStorage`에 보관되던 임시 Draft가 자동 정리되며, 페이지 새로고침 후에도 동일 세션으로 복원할 수 있다는 점을 시연한다.
7. 제출 직후 `조치 내역` 패널을 열어 폐기 사유·경고 메모·미등록 폐기 기록이 남겨진 것을 확인한다. `inspection_action_item`에 저장된 스냅샷 정보를 강조하고, 동일 데이터가 거주자 알림·벌점 처리와 연결될 것임을 짚는다.
8. 확장 계획으로 [MVP Plan IN-306](mvp-plan.md#in-306-실시간-검사-합류복구)에 따라 단일 SSE 스트림으로 2인 합류를 지원할 예정임을 언급하되, 현재 데모는 단독 검사 안정화에 초점을 둔다. (현재는 30~60초 간격 폴링으로 잠금/세션 상태를 재조회)
...
2. 알림을 클릭하면 `correlationId`로 연결된 검사 결과 상세 화면으로 이동한다. 방금 폐기된 포장이 강조되며, 경고/폐기 건수가 알림 본문과 일치하는지 확인한다. 메타데이터에는 `actionIds/actionCorrelationIds`, `penaltyCorrelationIds`가 모두 담겨 추적 가능함을 확인한다.
