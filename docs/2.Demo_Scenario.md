...
### 2.4 자동화 명령 요약
- `./auto db migrate --env prod` : Flyway 마이그레이션 적용 (필요 시 `--repair`)
- `./auto deploy up --build --env prod` : 최신 컨테이너 빌드·기동
- `./auto deploy reset --build --env prod` : down --volumes → db/redis → migrate → up 순으로 초기화
- `./auto deploy tls issue|renew --env prod` : HTTPS 인증서 발급/갱신 (도메인 사용 시)
