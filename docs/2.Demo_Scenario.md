...
* (시연 멘트)
    ...
    *(층별장 화면에서 '검사 시작' 클릭)*
    ...
    "이 순간, `InspectionService`가 호출되어 냉장고 칸이 30분간 'IN_INSPECTION' 상태로 잠깁니다. 30분이 지나면 5분 주기 스케줄러가 만료된 잠금을 찾아 세션을 `CANCELLED`로 돌리고 칸을 해제해, 무한 잠금을 방지합니다. `scheduleId`를 넘기면 해당 검사 일정에 링크됩니다."
    뒤로가기로 임시 저장이 자동 복원되는 기능은 없습니다. 세션을 수동 취소(`DELETE /fridge/inspections/{id}`)하면 일정 링크가 해제되고 상태가 `SCHEDULED`로 돌아가 다시 잡을 수 있습니다.
...
> **향후 대응**  
> 같은 계정을 다른 사람이 다른 기기에서 사용해 검사·조치를 수행하는 위험을 줄이기 위해, 차기 구현에서는 세션 기기 변경 알림/이중 승인, 관리자 강제 로그아웃 같은 방어 수단을 추가할 계획입니다.
