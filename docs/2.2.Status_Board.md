# 진행/상태 보드
*용도: `docs/2.1.Demo_Plan.md`에서 선택한 작업의 세부 진행 상황과 테스트 결과를 기록하는 로그 보드로, 완료 후 근거를 남길 때 사용한다.*

현재 작업의 완료 여부와 다음 우선순위를 간단히 체크한다.

## 사용 지침
- 진행 절차: `docs/2.1.Demo_Plan.md`에서 항목 선택 → 여기(Status Board)에 WIP로 요구사항·세부 작업·테스트 계획을 기록 → 작업/테스트 완료 후 PASS/FAIL 근거 남김 → `docs/2.1.Demo_Plan.md` 체크박스를 `[x]`로 갱신.
- 수행할 작업은 `docs/2.1.Demo_Plan.md`에서 선택하고, 세부 단계·테스트·결과를 이 보드에 기록한다.
- 개별 체크 항목을 모두 마무리한 뒤에만 `docs/2.1.Demo_Plan.md`의 체크박스를 `[x]`로 갱신한다.
...
### Backlog — 문서/코드 정합성
- **DOC-OPENAPI**: `api/openapi.yml`/`api/versions/v0.1.1.yml`을 최신 컨트롤러 기준으로 유지하고, `docs/2.1.Demo_Plan.md`, `docs/2.Demo_Scenario.md` 링크를 동기화할 것. 현재 일부 엔드포인트/필드(dedupe 키, allow_background, inspection submit, admin 재배분) 미반영 여부 확인.
- **DOC-NOTI-POLICY**: 알림 정책 테이블(`notification_policy`)은 미도입 상태. 현재 `admin_policy`로 TTL/한도 관리 중임을 문서화하고, 테이블 도입·UI 확장은 Post-MVP Backlog에 유지.
- **DOC-RESET**: 비밀번호 재설정/계정 잠금 해제 API 미구현. `docs/ops/security-checklist.md`·`docs/2.1.Demo_Plan.md`에 “향후 구현 시 재검증”으로 명시하고 AU-103에 연결.
- **DOC-SSE**: 실시간 검사 합류(SSE) 비활성. 관련 시나리오/테스트는 IN-306 Backlog로 유지하고 현행은 폴링/단독 검사 기준으로만 기술.
- **DOC-AUTH-DEVICE**: deviceId 불일치 시 `DEVICE_MISMATCH` 폐기 흐름/TTL(access 45s, refresh 5m 데모 기준)을 ops/security·시나리오/플랜에 반영 완료 여부 점검.

### (Backlog) IN-306 실시간 검사 합류·복구 — Stretch
 - **비고**: MVP 범위 밖 확장 과제. SSE 합류·재연결 설계 노트는 [docs/2.1.Demo_Plan.md](2.1.Demo_Plan.md#in-306-실시간-검사-합류복구)와 [docs/1.Feature_Inventory.md](1.Feature_Inventory.md#실시간-협업-및-복구)에 정리되어 있으며, 차기 일정에서 착수한다.
...
- **세부 작업**
  ...
  - `deploy/.env.prod`, `docker-compose*.yml` 간 변수 매핑·비상 전환 플랜 점검(`2.Demo_Scenario.md §4` 참고) — 로컬/데모 환경 전환 시 `docker compose --env-file deploy/.env.prod down && docker compose --env-file deploy/.env.prod up -d` 및 `flywayMigrate` 재실행 절차 정리.
- **테스트 및 확인 로그**
...
- **근거 문서**: `2.Demo_Scenario.md §3.1`, `1.Feature_Inventory.md §2`, `2.1.Demo_Plan.md:FR-201`
- **현행 파악**:
...
- **근거 문서**: `2.Demo_Scenario.md §3.1`(거주자 등록 1~4단계), `1.Feature_Inventory.md §2`(허용량·라벨 정책), `data-model.md §4.2`(칸 용량·라벨 시퀀스).
- **현행 파악**
...
- **근거 문서**: `2.Demo_Scenario.md §3.2`, `1.Feature_Inventory.md §2`, `docs/2.1.Demo_Plan.md:IN-304`
- **현행 파악**
...
- **근거 문서**: `docs/design/admin-wireframes.md §6`, `docs/data-model.md §4.1`, `docs/1.Feature_Inventory.md §상벌점 및 감사`
- **범위**
...
- **근거 문서**: `2.1.Demo_Plan.md:FR-204`, `2.Demo_Scenario.md §3.3`
- **변경 사항**
...
- **근거 문서**: `2.1.Demo_Plan.md:NO-401~403`
- **변경 사항**
...
- **근거 문서**: `2.1.Demo_Plan.md:SC-401`, `1.Feature_Inventory.md §3`, `frontend/app/_components/home/use-home-state.ts`
- **현행 파악**
...
- **진행 로그**
  ...
  - 2025-11-02: 관리자 화면은 조회 전용으로 유지하고, Post-MVP에서 관리자 일정 등록·알림 플로우를 확장하기 위한 메모를 `2.Demo_Scenario.md`에 기록했다.
...
- **근거 문서**: `2.1.Demo_Plan.md:AD-601`, `docs/design/admin-wireframes.md §1~4`, `1.Feature_Inventory.md §5 냉장고 – 관리자`, `docs/ops/batch-notifications.md`
- **현행 파악**
...
- **근거 문서**: `2.1.Demo_Plan.md:AD-602`, `docs/design/admin-wireframes.md §6`, `1.Feature_Inventory.md §5`
- **현행 파악**
...
- **근거 문서**: `2.1.Demo_Plan.md:AD-603`, `2.Demo_Scenario.md §3.5`, `ai-impl/frontend.md` 관리자 IA 섹션
- **현행 파악**
  - 관리자 화면은 거주자·층별장과 동일한 하단 탭을 공유하되, `관리(⚙️)` 허브에서 자원/권한/알림/벌점/리포트를 중앙 집중 관리하는 하이브리드 IA로 합의되었다(`docs/2.1.Demo_Plan.md:279`).
...
- **진행 로그**
- 2025-11-01: `docs/2.1.Demo_Plan.md:279`에 하이브리드 IA 합의 메모 추가.
...
- **근거 문서**: `2.1.Demo_Plan.md` 냉장고 증설 체크리스트, `1.Feature_Inventory.md §5` 냉장고 관리자 정책, `docs/data-model.md` `compartment_room_access`
- **현행 파악**:
...
- **근거 문서**: `docs/2.1.Demo_Plan.md` NO-403, `1.Feature_Inventory.md §3`(알림 & 일정)
- **세부 작업**
...
- **진행 로그**
  ...
  - 2025-11-12: `1.Feature_Inventory.md §3`, `nogitReadME.md` 알림 항목을 재검토해 알림 종류·TTL·설정 옵션 목록을 정리하고 헤더 드롭다운 UX 요구사항 초안을 수립.
...
- **근거 문서**: `docs/2.1.Demo_Plan.md` NO-501, `1.Feature_Inventory.md §3`
- **세부 진행**
...
- **근거 문서**: `docs/2.1.Demo_Plan.md (IN-303 확장)`, `docs/1.Feature_Inventory.md §2`, `docs/design/admin-wireframes.md §17`
- **변경 사항**
