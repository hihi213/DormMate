...
### 오류 응답 사전
- 모든 API는 `code` 필드와 HTTP Status를 포함한 `ProblemDetail` 형태로 응답하며, 프런트는 전역 오류 사전(`frontend/lib/api-errors.ts`)을 통해 도메인별 공통 메시지를 출력한다.
...
### 7.4 공통
- 하루 알림 제한, 상벌점/감사 로그 연동은 Phase 2 모듈에도 동일하게 적용한다.
