# DormMate 기능 인벤토리
*목적: DormMate 서비스의 기능, 정책, 용어를 모듈별로 정리한 기준 문서이다. 구현 일정·우선순위는 `docs/demo-scenario.md`를 참고한다.*

> **문서 사용 가이드**
> - 각 섹션은 특정 모듈의 기능 정의를 다룬다
> - 구현과 mvp범위는 `docs/mvp-scenario.md`를 참고한다.
...
### 역할
- 층별장은 담당 층의 냉장고 검사를 수행하고, 검사 결과를 기록·전파한다. 필요 시 2명이 동시에 검사에 참여할 수 있다.

### 기능 정의
- **검사 세션**: 담당 층 칸만 검사 시작 가능하며, 세션 생성 시 해당 칸을 잠금 상태로 전환한다. 동일 칸 검사 중일 경우 “검사 합류”로 참여할 수 있으며 최대 2인을 기본으로 한다.
- **실시간 동기화**: SSE 기반으로 조치 결과를 공유한다. 네트워크 문제 시 단독 검사 플로우로 전환하는 폴백 전략을 마련한다.
