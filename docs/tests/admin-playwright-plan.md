...
4. 포장 목록 검색 QA
   - 다자릿 슬롯 코드(`AA`, `AA-101`)를 검색했을 때 해당 슬롯 포장만 표기되는지 확인.
   - 보관자 이름, 호실(`3F 301`) 입력 시 각각 매칭되는 행만 남아 있는지 확인.
   - 검색 후 초기화 버튼을 눌러 페이지/검색어가 1페이지로 리셋되는지 검증.
5. 라벨 재사용 배지 확인
   - 같은 라벨 번호를 가진 삭제 이력이 있을 때 `라벨 재사용` 배지가 표시되는지 확인.
   - 최근 삭제 이력이 없는 라벨에는 배지가 노출되지 않는지 검증.
